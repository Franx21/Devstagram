    ...
    <div class="md:flex md:justify-center md:gap-10 md:items-center p-5">
        ...
        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
            <form method="POST" action="{{ route('login') }}" novalidate>
                @csrf
                @if (session('mensaje'))
                    <p class="bg-red-500 my-2 p-2 text-center text-white border rounded-lg">{{ session('mensaje') }}</p>
                @endif
                <div class="mb-5">
                ...
                <div class="mb-5">
                    <input type="checkbox" name="remember"><label class="text-gray-500 text-sm">Mantener la sesion
                        abierta</label>
                </div>
